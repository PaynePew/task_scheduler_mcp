...
- **Status**: Accepted
- **Date**: 2026-05-12
- **Source**: internal grilling session Q7 (local-only, not in git)
...
- If SQS send fails after the row is updated to QUEUED, the row sits stuck. This is addressed by the reconciler (Sweep B in `app/workers/reconciler.py`): a periodic background sweep re-enqueues `QUEUED` rows whose `updated_at` exceeds the `RECONCILER_QUEUED_GRACE_SECONDS` window **and** whose `scheduled_at` is already past — a row the Watcher queued ahead of time via its lookahead window (with an SQS `DelaySeconds`) sits QUEUED with a stale `updated_at` on purpose and must not be re-enqueued early (issue #30, guard added for issue #269).
