...
- **Effectively-once for non-idempotent sends (ADR-070).** A non-idempotent action re-executed on the same `run_id` (SQS `RETRYING` redelivery, reconciler retry) must not repeat its external side effect. The pattern (`app.actions.send_dedup`, wired for `email_send`): a **run-derived idempotency key** (`derive_idempotency_key(action, run_id)`), a **write-ahead intent** row (`send_intents`, PK'd on the key) written `attempting` **before** the provider call, and `mark_sent(key, provider_message_id)` committed in its own transaction **immediately after** the confirmed send (before returning to the executor — that write, not the terminal status, is the dedup arbiter). On replay the pure `decide_send` gate maps the stored status → `send` / `skip` (already `sent` → no-op success) / `resend`. Never key dedup on message *content* (collides across legitimately-identical sends); key it on the run. The one residual window ("sent, then crashed before the `sent` write") is documented in ADR-070: `email_send` biases to at-least-once and tolerates the rare duplicate. This is **effectively-once, not exactly-once** — do not claim the latter for an external side effect.
...
- Long-running actions extend SQS visibility via `heartbeat` (every 30s). A crashed worker's message must be allowed to fail over to another worker — never delete a message before the handler returns success. The heartbeat now also renews a **DB-side lease**: the claim sets `job_runs.heartbeat_at = now()` at the `RUNNING` transition, and the same heartbeat tick that extends SQS visibility also bumps `heartbeat_at` (issue #267). Unlike `updated_at` (frozen at claim), `heartbeat_at` stays fresh for the life of a legitimately long-running action — it is the observable the `RUNNING`-orphan sweep keys on.
- **Reconciler Sweep C reclaims dead-worker `RUNNING` orphans (ADR-069, issue #271).** A `RUNNING` row whose `heartbeat_at` lease is older than `reconciler_running_grace_seconds` (or NULL) is a dead-worker orphan — the redelivered message cannot re-claim it (claim rejects `RUNNING`), so the reconciler is the only recovery. Recovery is **keyed on the action's `idempotent` posture**, never a hardcoded action-name check: non-idempotent → `FAILED` + `RunEvent(FAILED, reason='running_orphan')` + `settle_job` (frees quota) + an operator-visible `logger.error` alert; idempotent → reset to a **claimable** status (`QUEUED`) + re-enqueue. The reset MUST be committed **before** the re-sent message is visible — a message that hits a still-`RUNNING` row is deleted as a duplicate and re-orphans the run, so commit-then-send, never send-then-commit. `running_grace` MUST exceed `heartbeat_interval(30s) + visibility(60s) = 90s` so a slow-but-alive long-running worker is never swept (default 180s = 2× that floor). Sweep C recovers only *dead*-worker orphans (stale lease); it never touches a live `RUNNING` run — cancel stays graceful (ADR-022). Re-introducing an `updated_at`/`start_at`-age `RUNNING` sweep (which would kill live long-running actions) is a review block.
