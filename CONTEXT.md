...
- **`effectively-once`** — the delivery guarantee for a non-idempotent external side effect (`email_send`): at-least-once SQS delivery + an **idempotent consumer at our boundary**. Not exactly-once — that is impossible end-to-end for an external side effect (two-generals). Implemented (ADR-070) with a **run-derived idempotency key** (`f"{action}:{run_id}"`), a **write-ahead intent** row in `send_intents` (written `attempting` before the provider call, flipped to `sent` + `provider_message_id` after), and a dedup no-op on replay of an already-`sent` key. One residual window ("sent, then crashed before the `sent` write") remains: `email_send` biases to **at-least-once and tolerates an extremely rare duplicate**; a future high-stakes flag flips it to at-most-once + alert.
- **`send intent` / `send_intents`** — the durable dedup record (one row per logical send, PK'd on the idempotency key) backing `effectively-once`. Separate from `JobRun` (the execution attempt) and `RunEvent` (the transition log): it records *whether this side effect has already happened*, not run state. No new run status — the run status machine (§2) is unchanged.

## §9 Security & trust boundaries

- **`ownership invariant`** — a user may only read or act on their own jobs and runs. Enforced on the **control plane** (`trigger_on_job_id` → `chain_validation` V2, ADR-020) **and** the **data plane** (`from_run_id` → `read_upstream` filters `JobRun.user_id`, ADR-071). A cross-tenant id resolves to "not found" (`NoResult` / 404-equivalent), never a distinct error, to prevent id enumeration.
- **`control plane` vs `data plane`** — control plane = scheduling/chaining fields the server validates at create (`trigger_on_job_id`, schedule, quotas). Data plane = fields that flow a prior run's output into a handler (`from_run_id` + `digest_v1`, ADR-033). Both must honour the `ownership invariant`; the data plane was the gap ADR-071 closed.
- **`credential track`** (recap of ADR-050) — public users act via their **own OAuth connections**; the operator acts via **`${VAR}` env substitution**. **Public actions never substitute `${VAR}`** (ADR-071 removed it from `email_send`); `${VAR}` lives only in operator-only actions, which are **fail-closed when `OPERATOR_USER_ID` is unset**. The hosted instance has no operator, so that track is dormant there.
- **`fixed-prompt transform`** — the LLM actions (`llm_summarize` / `llm_polish`) are a constrained, cost-capped transform, not a general generator (ADR-052). User-controlled `language`/`focus` are length-bounded and newline-stripped (ADR-071) so they cannot inject into the system prompt or make it free-form.
- **`untrusted input → sink`** — `action_params` are untrusted. Sinks: external APIs (Gmail/Slack/GitHub, on the user's own token), the operator LLM, and `from_run_id`. Residual, accepted: **indirect prompt injection** — poisoned upstream content summarized then delivered to the user's **own** sink; blast radius is the user's own destination (see ADR-071 residual risks).
